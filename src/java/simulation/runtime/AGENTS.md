...
## Scope
- This file applies to `src/java/simulation/runtime/` unless a deeper `AGENTS.md` overrides it.
...
- `CombatActionResolver.java`: resolves attack actions into damage, ICD-gated elemental application, aura changes, reactions, additive reaction bonuses, Dendro Core consumption, and follow-up effects without advancing time.
...
- `ReactionState.java`: simulator-owned transient reaction state, including Electro-Charged timer state, Thundercloud, Burning, Quicken, Dendro Cores, Verdant/Moonridge Dew, Moondrift counters, and Lunar-Crystallize trigger counts.
...
- Snapshot/restore in `CombatSimulator` and `SimulatorSnapshot` must stay aligned with fields added to `ReactionState`.
...
- When editing `CombatActionResolver` or `ReactionState`, run `./gradlew ReactionRegressionTest` before sample simulations.
