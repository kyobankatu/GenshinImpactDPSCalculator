package sample;

import model.stats.StatsContainer;
import model.type.StatType;
import model.type.WeaponType;
import model.weapon.BoundaryInactiveWeapon;
import model.weapon.ProspectorsDrill;
import model.weapon.PrototypeCrescent;
import model.weapon.TalkingStick;
import model.weapon.TheDockhandsAssistant;

/** Table-driven regression checks for unavailable-state weapons. */
public final class UnavailableStateWeaponRegressionTest {
    private static final double EPSILON = 1e-8;

    private UnavailableStateWeaponRegressionTest() {
    }

    /** Runs metadata, boundary, and refinement guard checks. */
    public static void main(String[] args) {
        for (WeaponCase weaponCase : cases()) {
            testCase(weaponCase);
        }
        System.out.println("UnavailableStateWeaponRegressionTest passed");
    }

    private static void testCase(WeaponCase weaponCase) {
        BoundaryInactiveWeapon defaultWeapon = weaponCase.create(5);
        assertEquals(weaponCase.name, defaultWeapon.getName(),
                weaponCase.name + " display name");
        assertEquals(weaponCase.type, defaultWeapon.getWeaponType(),
                weaponCase.name + " type");
        assertClose(weaponCase.baseAtk,
                defaultWeapon.getStats().get(StatType.BASE_ATK),
                weaponCase.name + " base ATK");
        assertClose(weaponCase.substatValue,
                defaultWeapon.getStats().get(weaponCase.substat),
                weaponCase.name + " substat");
        for (int refinement = 1; refinement <= 5; refinement++) {
            BoundaryInactiveWeapon weapon = weaponCase.create(refinement);
            assertEquals(refinement, weapon.getRefinement(),
                    weaponCase.name + " selected refinement");
            for (double time : new double[] { -10.0, 0.0, 100.0 }) {
                StatsContainer stats = sentinelStats();
                weapon.applyPassive(stats, time);
                assertClose(0.25, stats.get(StatType.ATK_PERCENT),
                        weaponCase.name + " preserves ATK");
                assertClose(80.0, stats.get(StatType.ELEMENTAL_MASTERY),
                        weaponCase.name + " preserves EM");
                assertClose(0.30, stats.get(StatType.DMG_BONUS_ALL),
                        weaponCase.name + " preserves damage");
            }
        }
        assertThrows(IllegalArgumentException.class,
                () -> weaponCase.create(0),
                weaponCase.name + " rejects R0");
        assertThrows(IllegalArgumentException.class,
                () -> weaponCase.create(6),
                weaponCase.name + " rejects R6");
    }

    private static WeaponCase[] cases() {
        return new WeaponCase[] {
            new WeaponCase(
                    "The Dockhand's Assistant",
                    WeaponType.SWORD,
                    510.0,
                    StatType.HP_PERCENT,
                    0.413,
                    TheDockhandsAssistant::new),
            new WeaponCase(
                    "Prospector's Drill",
                    WeaponType.POLEARM,
                    565.0,
                    StatType.ATK_PERCENT,
                    0.276,
                    ProspectorsDrill::new),
            new WeaponCase(
                    "Prototype Crescent",
                    WeaponType.BOW,
                    510.0,
                    StatType.ATK_PERCENT,
                    0.413,
                    PrototypeCrescent::new),
            new WeaponCase(
                    "Talking Stick",
                    WeaponType.CLAYMORE,
                    565.0,
                    StatType.CRIT_RATE,
                    0.184,
                    TalkingStick::new)
        };
    }

    private static StatsContainer sentinelStats() {
        StatsContainer stats = new StatsContainer();
        stats.set(StatType.ATK_PERCENT, 0.25);
        stats.set(StatType.ELEMENTAL_MASTERY, 80.0);
        stats.set(StatType.DMG_BONUS_ALL, 0.30);
        return stats;
    }

    private static void assertClose(
            double expected,
            double actual,
            String message) {
        if (Math.abs(expected - actual) > EPSILON) {
            throw new AssertionError(message + ": expected " + expected
                    + " but got " + actual);
        }
    }

    private static void assertEquals(
            Object expected,
            Object actual,
            String message) {
        if (!expected.equals(actual)) {
            throw new AssertionError(message + ": expected " + expected
                    + " but got " + actual);
        }
    }

    private static void assertThrows(
            Class<? extends Throwable> expected,
            ThrowingRunnable action,
            String message) {
        try {
            action.run();
        } catch (Throwable throwable) {
            if (expected.isInstance(throwable)) {
                return;
            }
            throw new AssertionError(message + ": expected "
                    + expected.getSimpleName() + " but got "
                    + throwable.getClass().getSimpleName(), throwable);
        }
        throw new AssertionError(message + ": expected "
                + expected.getSimpleName());
    }

    @FunctionalInterface
    private interface WeaponFactory {
        BoundaryInactiveWeapon create(int refinement);
    }

    @FunctionalInterface
    private interface ThrowingRunnable {
        void run() throws Exception;
    }

    private static final class WeaponCase {
        private final String name;
        private final WeaponType type;
        private final double baseAtk;
        private final StatType substat;
        private final double substatValue;
        private final WeaponFactory factory;

        private WeaponCase(
                String name,
                WeaponType type,
                double baseAtk,
                StatType substat,
                double substatValue,
                WeaponFactory factory) {
            this.name = name;
            this.type = type;
            this.baseAtk = baseAtk;
            this.substat = substat;
            this.substatValue = substatValue;
            this.factory = factory;
        }

        private BoundaryInactiveWeapon create(int refinement) {
            return factory.create(refinement);
        }
    }
}
