...
public final class RotationSourceCatalogRegressionTest {
    ...
    private static void assertTrackedPilotCatalogRoundTrips() throws Exception {
        ...
        if (catalog.getSources().size() != 73 || catalog.getSeeds().size() != 71) {
            throw new AssertionError("Expected seventy-three sources and seventy-one seeds");
        }
        ...
        if (rejectedCount != 27) {
            throw new AssertionError("Expected twenty-seven unavailable pilot seeds");
        }
        assertEquals(43, catalog.getUsableSeeds(DatasetSplit.TRAIN).size(), "train seeds");
