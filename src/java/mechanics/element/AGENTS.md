...
- `ICDManager.java`: tracks elemental application internal cooldown state by character identity and ICD tag, including standard hit/time rules, no-ICD behavior, and shared tag gating.
...
## Agent guidance
- If you change aura application behavior, inspect `ICDManager`, `simulation.action.AttackAction`, `simulation.runtime.CombatActionResolver`, and `model.entity.Enemy`.
- Damage and elemental application are deliberately separable: a hit may still deal damage while ICD blocks aura application.
