...
## Scope
- This file applies to `src/mechanics/element/`.
...
- `ICDManager.java`: tracks elemental application internal cooldown state by character and ICD tag.
...
- `ICDManager` depends on `model.type.ICDType` and `model.type.ICDTag` and is used by `simulation.CombatSimulator` when deciding whether a hit applies aura.
