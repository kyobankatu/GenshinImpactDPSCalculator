...
- Representative runnable entry points are in `src/sample/` such as `RaidenParty`, `FlinsParty`, `FlinsParty2`, `TrainRLJava`, and `EnjoyRLJava`.
...
- Run Java-native RL training: `./gradlew TrainRLJava`
- Run a trained Java-native RL policy and generate an RL report: `./gradlew EnjoyRLJava`
...
- If your change affects RL integration, verify the Java-native training/evaluation entry point you touched.
...
- RL training runs in-process on Java-native environment classes. Do not reintroduce external network dependencies into the RL path unless explicitly requested.
