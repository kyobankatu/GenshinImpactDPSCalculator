...
- Use `discover-genshin-work` only when every active `TASKS.md` plan phase is done and no requested work remains. It sweeps approved sources against the `BACKLOG.md` ledger, applies a value and risk gate, records rejections, treats `## Deferred Systems` as forbidden, and converges instead of manufacturing work.
- `BACKLOG.md` is the durable ledger of discovered work items and settled decisions. `TASKS.md` holds only work that is being done; candidate, rejected, blocked, and deferred items belong in the ledger.
